...
RSpec.describe 'Book Tagging API', type: :request do
  describe 'POST #create' do
    context 'with valid params' do
      it 'adds a tag to a book' do
        ...
        post book_tags_url(book), params: { name: tag.name }, headers: acting_as(create(:user, :admin))
    ...
    context 'with invalid params' do
      it 'renders a JSON response with errors for the tag' do
        ...
        post book_tags_url(book), params: { name: nil }, headers: acting_as(create(:user, :admin))
  ...
  describe 'DELETE #destroy' do
    it 'removes a tag from a book' do
      ...
      delete book_tag_url(book, tag), headers: acting_as(create(:user))
